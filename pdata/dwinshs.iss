...
function dw_PeekMessage(var Msg: dw_TMsg; Wnd: HWnd; MsgFilterMin, MsgFilterMax, wRemoveMsg: LongInt): BOOL;
external 'PeekMessageW@user32.dll stdcall delayload setuponly';

function dw_TranslateMessage(var Msg: dw_TMsg): BOOL;
external 'TranslateMessage@user32.dll stdcall delayload setuponly';

function dw_DispatchMessage(var Msg: dw_TMsg): LongInt;
external 'DispatchMessageW@user32.dll stdcall delayload setuponly';

procedure DwinsHs_ProcessMessages();
var
  Msg: dw_TMsg;
begin
  while dw_PeekMessage(Msg, 0, 0, 0, 1) do
  if Msg.Msg <> $0012 then
  begin
    dw_TranslateMessage(Msg);
    dw_DispatchMessage(Msg);
  end
  else
  begin
